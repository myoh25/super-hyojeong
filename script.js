document.addEventListener('DOMContentLoaded', () => {
  ...
  function createPlayerBullet() {
    if (isSuperActive) {
      // 슈퍼웨폰: 30발, 360도, 속도 15
      const num = 30, spread = 360, startA = 0, speed = 15;
      for (let i = 0; i < num; i++) {
        const angle = startA + i * (spread / num);
  ...
  window.addEventListener('pointermove', e => {
    ...
    let cx = player.offsetLeft + player.offsetWidth/2 + dx;
    let cy = player.offsetTop  + player.offsetHeight/2 + dy;
